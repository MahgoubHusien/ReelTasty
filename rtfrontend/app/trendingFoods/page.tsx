...
import { fetchVideos, fetchRecentlySeenVideos, fetchUserId } from "@/service/api";
import { VideoMetaData } from "@/types/types"; 

const hashtags = ["tiktokfood", "food", "cooking", "foodie", "foodtok", "recipesoftiktok", "baking", "easyrecipe", "comfortfood", "healthyfood", "cookwithme"];

const TrendingFoodsPage: React.FC = () => {
  const [videos, setVideos] = useState<{ [key: string]: VideoMetaData[] }>({});
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      try {
        ...
        const recentlySeen = await fetchRecentlySeenVideos();
